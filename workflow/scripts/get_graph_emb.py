...
import fastnode2vec
from infopolluter.wrappers import Timer
...
def pecan(
    ...
    # edge_fp: tab-separated edgelist
    if mode == "PreComp":
        graph = pecanpy.PreComp(p=p, q=q, workers=workers, verbose=verbose, extend=True)
    else:
        graph = pecanpy.DenseOTF(
            p=p, q=q, workers=workers, verbose=verbose, extend=True
        )

    graph.read_edg(edge_fp, weighted=weighted, directed=directed)
    if mode == "PreComp":
    ...
    walks = graph.simulate_walks(**walk_params)
...
def main(args):
    parser = argparse.ArgumentParser(
        description="Get graph embedding",
    )
    parser.add_argument(
        "-i",
        "--infile",
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="node2vec embedding package to use: {fast, elio, pecan}",
    )
    ...
    # weighted = True if args.weighted else False
    ...
    # reverse = True if args.reversed else False
    reverse = True if direction == "trust" else False

    if args.embedding == "pecan":
