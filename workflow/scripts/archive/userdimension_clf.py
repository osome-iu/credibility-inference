...
def rank_with_hidden_labels_locred(
    ...
    options:
    [mistrust, both=False: get 1 score in rt network: personalized on good or bad]
    [mistrust, both: get 2 scores in rt network: personalized on good, bad]
    [trust, both: get 2 scores in trust network: personalized on good, bad]
    [both, both: get 4 scores (2 from rt net and 2 from trust net)]
    """
...
if __name__ == "__main__":
    ...
    EXP_PATH = "/N/u/baotruon/Carbonate/infopolluters/exps"

    rtedgelist = os.path.join(DATA_PATH, "rt_cc_size322208.txt")
    trustedgelist = os.path.join(DATA_PATH, "trust_cc_size322208.txt")
    labelfile = os.path.join(DATA_PATH, "user_labels.csv")
    labels = iputils.read_label_csv(labelfile)

    params = {"score_type": "both", "complement": False, "kfold": 10}
    legal_params = utils.remove_illegal_kwargs(params, cross_eval_graphemb)
    report = cross_eval_graphemb(
        labels, rtedgelist=rtedgelist, trustedgelist=trustedgelist, **legal_params
    )

    outfile = os.path.join(EXP_PATH, "embnscore/user_dim.pkl")
    verboseoutfile = os.path.join(EXP_PATH, "embnscore/user_dim.json")
