...
url_dir = "/data_volume/midterm2022/intermediate_files/entities/meta/expanded_urls"
...
rule clf:
    ...
    shell: """
    python3 workflow/scripts/cocred_eval.py -i {input.graph} -u {input.user_labels} -o {output} --method {wildcards.method} -a {wildcards.alpha}
    """

rule user_labels:
    input:
        platforms = os.path.join(label_dir, "all_platforms.csv"),
    ...
    output:
        ...
        user_labels = os.path.join(label_dir, "user_info.parquet")
    shell:"""
    python3 workflow/preprocess/user_shared_domains.py -i {url_dir} -d {input.domain_labels} --platforms {input.platforms} --outpkl {output.user_domain} --outcsv {output.domain_idx} --outparquet {output.user_labels} -s {start} -e {end}
    """
...
rule make_edgelist:
    ...
    shell: """
    python3 workflow/preprocess/fb/bipartite_edgelist.py {input.input_dir} {input.platforms} {output} {start} {end}
    """
