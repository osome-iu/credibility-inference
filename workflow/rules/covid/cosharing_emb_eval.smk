...
edgelist_fname = "cosharing_edgelist__67539"
print(os.getcwd())

rule all:
    input:
        expand(os.path.join(res_dir, f"p{{p}}q{{q}}__{edgelist_fname}.pkl"), p=p_vals, q=q_vals)
...
rule clf:
    ...
    output: os.path.join(res_dir, f"p{{p}}q{{q}}__{edgelist_fname}.pkl")
    shell: """
