...
import os
import numpy as np
from credinference.util import warn
...
class ConvoKitMatrix:
    ...
    def matrix(self, value):
        if isinstance(value, np.ndarray) or isinstance(
            value, scipy.sparse.csr.csr_matrix
        ):
            self._matrix = value
    ...
    def get_vectors(
        ...
        if not as_dataframe:
            ...
            return pd.DataFrame(
                mat[id_indices][:, col_indices], index=ids, columns=cols
            )
    ...
    def from_dir(dirpath, matrix_name):
        ...
        try:
            with open(
                os.path.join(dirpath, "vectors.{}.p".format(matrix_name)), "rb"
            ) as f:
                retval: ConvoKitMatrix = pickle.load(f)
                if not retval._sparse:
                    retval.matrix = retval.matrix.toarray()
            ...
            warn(
                "Could not find vector with name: {} at {}.".format(
                    matrix_name, dirpath
                )
            )
            return None
    ...
    def dump(self, dirpath):
        ...
        if not issparse(self.matrix):
            ...
            with open(
                os.path.join(dirpath, "vectors.{}.p".format(self.name)), "wb"
            ) as f:
                pickle.dump(self, f)
            self.matrix = temp
        else:
    ...
    def subset(
        self, ids: Optional[List[str]] = None, columns: Optional[List[str]] = None
    ):
        """
    ...
    def hstack(name: str, matrices: List["ConvoKitMatrix"]):
        ...
        return ConvoKitMatrix(
            name=name, matrix=stacked, ids=matrices[0].ids, columns=columns
        )
    ...
    def vstack(name: str, matrices: List["ConvoKitMatrix"]):
        ...
        return ConvoKitMatrix(
            name=name, matrix=stacked, ids=ids, columns=matrices[0].columns
        )

    def __repr__(self):
        return "ConvoKitMatrix('name': {}, 'matrix': {})".format(
            self.name, repr(self.matrix)
        )
